...
class ProjectProposalIndexService
{
    ...
    public function getProjects(Request $request, User $user): array
    {
        ...
        if ($request->filled('start_date')) $cacheKeyParams['start_date'] = $request->start_date;
        if ($request->filled('end_date')) $cacheKeyParams['end_date'] = $request->end_date;
    ...
    private function getSelectFields(): array
    {
        return [
            ...
            'phase_day', 'month_number', 'month_start_date', 'is_monthly_phase',
            'rejection_reason', 'rejection_message', 'admin_rejection_reason', 'media_rejection_reason',
    ...
    {
        $query->where(function($q) {
            $q->whereNull('is_divided_into_phases')->orWhere('is_divided_into_phases', false)->orWhere('is_daily_phase', true);
        });

        if ($request->filled('status') && $request->status !== 'all') {
            $query->whereIn('status', (array)$request->status);
        } else {
            $query->whereIn('status', [self::STATUS_ASSIGNED_TO_RESEARCHER, self::STATUS_READY, 'تم اختيار المخيم', self::STATUS_EXECUTING, self::STATUS_EXECUTED, self::STATUS_MONTAGE, self::STATUS_MONTAGE_COMPLETED, self::STATUS_MONTAGE_REDO, 'معاد مونتاجه', self::STATUS_DELIVERED])
    ...
    {
        $executedStatuses = [self::STATUS_EXECUTED, 'منفذ', self::STATUS_MONTAGE, self::STATUS_MONTAGE_COMPLETED, 'معاد مونتاجه', self::STATUS_DELIVERED];
    ...
    {
        $nowMonthStart = Carbon::now()->startOfMonth()->format('Y-m-d');
        $query->leftJoin('project_proposals as parent_proposals', 'parent_proposals.id', '=', 'project_proposals.parent_project_id');
        $query->where('project_proposals.project_type', 'الكفالات');
        
        $query->where(function ($scopeQ) use ($nowMonthStart) {
            $scopeQ->where(function ($nonDivided) {
                $nonDivided->whereNull('project_proposals.parent_project_id')->whereNull('project_proposals.month_number');
            })
            ->orWhere(function ($monthlyChild) use ($nowMonthStart) {
                $monthlyChild->whereNotNull('project_proposals.parent_project_id')
                    ->where(function ($fallback) use ($nowMonthStart) {
                        $fallback->whereBetween('project_proposals.month_start_date', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()]);
                    });
            });
        });

        $selectFields = $this->getSelectFields();
        $query->select(array_map(fn($f) => "project_proposals.$f", $selectFields));
        $query->whereIn('project_proposals.status', ['جديد', 'قيد التوريد', 'تم التوريد', 'مسند لباحث', 'جاهز للتنفيذ', 'قيد التنفيذ', 'تم التنفيذ', 'في المونتاج', 'تم المونتاج', 'وصل للمتبرع'])
              ->where('project_proposals.status', '!=', self::STATUS_COMPLETED);
    }
    ...
    private function applyAdminFilters(Builder $query, Request $request): void
    {
        ...
        if (!empty($searchQuery)) $this->applySearchFilter($query, trim($searchQuery));
    }

    private function applyCommonFilters(Builder $query, Request $request, string $userRole): void
    {
        if ($userRole === self::ROLE_ADMIN) return;

        if ($request->filled('subcategory_id')) $query->whereIn('subcategory_id', (array)$request->subcategory_id);
        if ($request->filled('team_id')) $query->where('assigned_to_team_id', $request->team_id);
    ...
    {
        return ProjectProposal::select([
            'id', 'serial_number', 'donor_code', 'internal_code', 'project_name', 
            'project_description', 'donor_name', 'status', 'execution_date', 
            'is_urgent', 'phase_day', 'is_daily_phase', 'is_monthly_phase', 'month_number', 'parent_project_id',
            'donation_amount', 'net_amount', 'amount_in_usd', 'currency_id',
            'assigned_montage_producer_id', 'assigned_photographer_id', 'assigned_researcher_id',
            'assigned_to_team_id', 'subcategory_id', 'shelter_id',
            'rejection_reason', 'media_rejection_reason', 'admin_rejection_reason', 
            'created_at', 'updated_at'
        ])
        ->with([
    ...
    private function buildCachedResponse(array $cachedData, string $cacheKey, ?string $requestEtag, int $cacheTtl): array
    {
        ...
        if ($requestEtag === $etag) return ['response' => response()->json([], 304)->header('ETag', $etag)];
