...
class StoreProjectProposalRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $merge = [];
        if (!empty($pd['type']))
            $merge['phase_type'] = $pd['type'];
        if (isset($pd['total_months']))
            $merge['total_months'] = $pd['total_months'];
        if (!empty($pd['phase_start_date']))
            $merge['phase_start_date'] = $pd['phase_start_date'];
        if (isset($pd['phase_duration_days']))
            $merge['phase_duration_days'] = $pd['phase_duration_days'];
