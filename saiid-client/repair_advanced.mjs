...
// We know line 1146 (approximately) has the Add Orphans button
let startIndex = -1;
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('setCurrentGroup(grouping);') && lines[i+1].includes('setShowAddOrphansModal(true);')) {
        // Found the button's onClick. Now find the end of this button.
        for (let j = i; j < lines.length; j++) {
            if (lines[j].trim() === '</button>') {
                startIndex = j + 1; // Start replacing AFTER the </button>
                break;
            }
        }
        break;
...
// Find the end of the groupings.map block
let endIndex = -1;
for (let i = lines.length - 1; i > 0; i--) {
    if (lines[i].includes('</tbody>') && lines[i+1].includes('</table>')) {
        // We are at the end of the table. The groupings.map should end just before this.
        for (let j = i; j > 0; j--) {
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    console.log('Successfully repaired advanced.jsx');
} else {
