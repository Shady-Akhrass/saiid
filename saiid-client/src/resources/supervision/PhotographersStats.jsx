...
const PhotographersStats = () => {
  ...
  const loadStats = async (refresh = false) => {
    ...
    try {
      ...
      console.error('Photographers Stats Error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const { total_statistics, averages, photographers } = statsData;

  // ✅ حساب نسبة الإنجاز العامة بشكل صحيح (fallback)
  const calculatedOverallCompletionRate = total_statistics?.total_assigned_projects > 0
    ? Math.round(((total_statistics?.total_completed_projects || 0) / total_statistics.total_assigned_projects) * 100 * 10) / 10
    : 0;
  const overallCompletionRate = averages?.overall_completion_rate !== undefined
    ? (Math.abs(averages.overall_completion_rate - calculatedOverallCompletionRate) < 0.1
      ? averages.overall_completion_rate
      : calculatedOverallCompletionRate)
    : calculatedOverallCompletionRate;

  return (
    <div className="photographers-stats min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-6" style={ { fontFamily: 'Cairo, Tajawal, Arial, sans-serif' } }>
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-1">
                    تحليل شامل لأداء وإنتاجية جميع المصورين (المشاريع المسندة فقط)
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-5 border-2 border-green-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-3xl font-bold text-green-600">
                      { total_statistics.total_photographers || 0 }
                    </span>
                  </div>
                  <p className="text-gray-700 font-semibold">إجمالي المصورين</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border-2 border-blue-200">
                  ...
                  <p className="text-gray-700 font-semibold">المشاريع المسندة للمصورين</p>
                ...
                {/* المشاريع التي وصلت للمونتاج */ }
                <div className="bg-gradient-to-br from-cyan-50 to-sky-50 rounded-xl p-5 border-2 border-cyan-200">
                  <div className="flex items-center justify-between mb-3">
                    <Zap className="w-8 h-8 text-cyan-600" />
                    <span className="text-3xl font-bold text-cyan-600">
                      { total_statistics.total_sent_to_montage || 0 }
                    </span>
                  </div>
                  <p className="text-gray-700 font-semibold">وصلت للمونتاج</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-5 border-2 border-indigo-200">
                  ...
                  <p className="text-xs text-gray-500 mt-1">المسندة للمصورين فقط</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-800">متوسط المشاريع لكل مصور</h3>
                  </div>
                  <p className="text-4xl font-bold text-indigo-600">
                    { averages.avg_projects_per_photographer?.toFixed(1) || 0 }
                  </p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-800">متوسط المشاريع المنجزة لكل مصور</h3>
                  </div>
                  <p className="text-4xl font-bold text-green-600">
                    { averages.avg_completed_per_photographer?.toFixed(1) || 0 }
                  </p>
                </div>
              </div>
            ) }
        ...
        { photographers && photographers.length > 0 && (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-800">أداء المصورين (مرتبون حسب إجمالي المشاريع)</h2>
            </div>
            ...
            <div className="space-y-4">
              { [...photographers]
                .sort((a, b) => {
                  ...
                  const totalA = a.statistics?.total_projects || 0;
                  const totalB = b.statistics?.total_projects || 0;
                  return totalB - totalA;
                })
                .map((photographer, index) => {
                  const stats = photographer.statistics || {};
                  ...
                  // ✅ حساب النسبة مباشرة من البيانات الفعلية (أكثر دقة)
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">{ photographer.phone_number }</p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-9 gap-3">
                        ...
                        <div className="bg-white rounded-lg p-3 text-center border border-cyan-200">
                          <p className="text-2xl font-bold text-cyan-600">{ stats.sent_to_montage || 0 }</p>
                          <p className="text-xs text-gray-600 mt-1">وصلت للمونتاج</p>
                        </div>
        ...
        { (!photographers || photographers.length === 0) && !loading && !error && (
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            <Camera className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-500 text-lg font-medium">لا توجد بيانات للمصورين</p>
          </div>
        ) }
      </div>
    </div>
