...
import { Search, Filter, X, ChevronDown, ChevronUp, Info } from 'lucide-react';
...
export const FilterBar = ({
  ...
  showMonthlyPhasesHelp = false,
  onToggleMonthlyPhasesHelp = null,
}) => {
  ...
  const hasActiveFilters = () => {
    return (
      ...
      filters.phase_day ||
      filters.parent_project_id ||
      filters.show_delayed_only ||
      filters.show_urgent_only ||
      filters.show_sub_projects_only ||
      filters.show_divided_parents_only ||
      filters.searchQuery
    );
  };
    ...
    <div className="bg-white rounded-3xl p-5 shadow-xl border border-gray-100">
      ...
      {showFilters && (
        <div className="pt-5 mt-5 border-t-2 border-gray-200">
          {isOrphanSponsorCoordinator && onToggleMonthlyPhasesHelp && (
            <div className="mb-4 rounded-2xl border-2 border-sky-100 bg-sky-50/80 p-4">
              <button
                type="button"
                onClick={onToggleMonthlyPhasesHelp}
                  ...
                  {showSubcategoryDropdown && !subcategoriesLoading && (
                    <div className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                      <div className="p-2">
                        ...
                        {subcategories.length === 0 && filters.project_type?.length > 0 ? (
                ...
                >
                  ...
                  {producers.map((producer) => (
                    <option key={producer.id} value={producer.id}>
                      {producer.name || producer.email || `المنتج ${producer.id}`}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex items-center">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className="relative">
                    <input
                      type="checkbox"
                      checked={filters.show_delayed_only || false}
            ...
            <div className="flex items-center">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative">
                  <input
                    type="checkbox"
                    checked={filters.show_urgent_only || false}
              ...
              <div className="flex items-center">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className="relative">
                    <input
                      type="checkbox"
                      checked={filters.show_sub_projects_only || false}
              ...
              <div className="flex items-center">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className="relative">
                    <input
                      type="checkbox"
                      checked={filters.show_divided_parents_only || false}
