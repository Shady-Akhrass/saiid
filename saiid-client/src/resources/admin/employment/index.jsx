...
const Employments = () => {
  const { getData, setCachedData, isCacheValid, initializeCache, clearCache } = useCache('employments', 300000); // 5 دقائق
  ...
  useEffect(() => {
    const handleCacheInvalidation = (event) => {
      ...
      if (cacheKey === 'all' || cacheKey === 'employments') {
        clearCache();
        setRefreshTrigger(prev => prev + 1);

        if (import.meta.env.DEV) {
          console.log('✅ Employments cache invalidated, fetching fresh data');
        }
      }
    };
  ...
  const fetchEmployments = async () => {
    try {
      ...
      if (isCacheValid(filtersKey)) {
        ...
        if (cachedData) {
          setEmployments(cachedData.employments || cachedData);
          setTotalEmployments(cachedData.totalEmployments || cachedData.length || 0);
          setTotalPages(cachedData.totalPages || Math.ceil((cachedData.totalEmployments || cachedData.length) / perPage) || 0);
          setIsLoading(false);
          if (import.meta.env.DEV) {
      ...
      const response = await apiClient.get("/employments", {
      ...
      if (Array.isArray(response.data.employments)) {
        ...
        setTotalEmployments(response.data.totalEmployments || 0);
        setTotalPages(response.data.totalPages || 0);

        // ✅ حفظ البيانات في cache
        setCachedData({
          employments: response.data.employments,
          totalEmployments: response.data.totalEmployments || 0,
      ...
      if (cachedData) {
        ...
        setIsLoading(false);
        if (import.meta.env.DEV) {
          console.log('⚠️ Error fetching employments, using cached data');
        }
        return;
      }
      ...
      setToast({
        message: "خطأ في جلب البيانات، يرجى المحاولة مرة أخرى.",
        type: "error",
        isVisible: true,
      });
      setEmployments([]);
  ...
  useEffect(() => {
    ...
    if (isCacheValid(filtersKey)) {
      ...
      if (cachedData) {
        ...
        setIsLoading(false);
        if (import.meta.env.DEV) {
          console.log('✅ Using cached employments data (from useEffect)');
        }
        return;
      }
    }
  ...
  const handleDownloadExcel = async () => {
    ...
    setToast({
      message: "جاري تحضير ملف Excel للتحميل...",
      type: "info",
      isVisible: true,
    });

    try {
      const response = await apiClient.get("/employments/export", {
        responseType: "blob",
      });
      ...
      setToast({
        message: "سيبدأ التحميل خلال لحظات...",
        type: "success",
        isVisible: true,
      });
    } catch (error) {
  ...
  const sortedEmployments = React.useMemo(() => {
    if (!sortConfig.key) return employments;
  ...
  const SkeletonRow = () => (
    <tr className="animate-pulse">
      ...
      <td className="p-4">
        <div className="h-6 bg-gradient-to-r from-sky-100 to-sky-200 rounded-full w-28 mx-auto"></div>
      </td>
      <td className="p-4">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">إجمالي السجلات: { totalEmployments }</p>
              </div>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-sky-50 to-orange-50 border-b border-sky-100">
                  ...
                  <th
                    onClick={ () => requestSort("previous_work_url") }
                    className="p-4 text-center font-semibold text-gray-700 cursor-pointer hover:bg-sky-100/50 transition-colors duration-300 group"
                  >
                    <div className="flex items-center justify-center gap-2">
              ...
              <tbody>
                { isLoading ? (
                  ...
                  sortedEmployments.map((employment, index) => (
                    <tr
                      key={ employment._id || `${employment.name}-${index}` }
                    ...
                    >
                      <td className="p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-semibold text-gray-800">{ employment.name }</p>
                            { employment.specialization && (
                              <p className="text-sm text-gray-500">التخصص: { employment.specialization }</p>
                            ) }
                          </div>
                        </div>
                      </td>
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ formatDate(employment.birth_date) }</span>
                      </td>
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ employment.address || "-" }</span>
                      </td>
                      <td className="p-4 text-center">
                      ...
                      <td className="p-4 text-center">
                        { employment.phone_number ? (
                          <a
                            href={ `tel:${employment.phone_number}` }
                      ...
                      <td className="p-4 text-center">
                        { employment.previous_work_url ? (
                          <a
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-400 to-orange-500 text-white rounded-xl hover:from-orange-500 hover:to-orange-600 transform hover:scale-105 transition-all duration-300 shadow-md"
                            title="عرض الأعمال"
                          >
