...
const OrphanGroupingsAdvanced = () => {
  ...
  const [groupViewModes, setGroupViewModes] = useState({}); // { groupId: 'table' | 'cards' }
  ...
  /**
   * Remove a single orphan from a group
   * @param {number|string} groupingId 
   * @param {string} orphanId 
   */
  const handleRemoveOrphanFromGroup = async (groupingId, orphanId) => {
    if (!window.confirm('هل أنت متأكد من إزالة هذا اليتيم من المجموعة؟')) {
      return;
    }
    
    try {
      const response = await apiClient.post(`/orphan-groupings/${groupingId}/remove-orphans`, {
        orphan_ids: [orphanId],
        status: 'inactive',
        notes: 'إزالة يدوية من إدارة المجموعات'
      });
      
      if (response.data.success) {
        success('تم إزالة اليتيم من المجموعة بنجاح');
        
        // Update local state for orphans in this group
        setGroupOrphans(prev => {
          const currentList = prev[groupingId] || [];
          return {
            ...prev,
            [groupingId]: currentList.filter(o => 
              !(o.orphan_id_number === orphanId || o.id === orphanId)
            )
          };
        });
        
        // Refresh grouping metadata (counts)
        fetchGroupings();
      } else {
        showError('فشل في إزالة اليتيم');
      }
    } catch (err) {
      console.error('Error removing orphan:', err);
      showError('حدث خطأ أثناء الإزالة: ' + (err.response?.data?.message || err.message));
    }
  };
                                          ...
                                          <tr key={orphan.orphan_id_number || orphan.id || idx} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3">
                                              <div className="flex gap-1">
                                                <button onClick={() => handleDownloadPdf(orphan.orphan_id_number || orphan.id_number)} className="p-1.5 text-red-600 hover:bg-red-50 rounded transition-colors" title="PDF"><FileText className="w-3.5 h-3.5" /></button>
                                                <button onClick={() => handleDownloadWord(orphan.orphan_id_number || orphan.id_number)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Word"><FileSpreadsheet className="w-3.5 h-3.5" /></button>
                                                <button onClick={() => handleRemoveOrphanFromGroup(grouping.id, orphan.orphan_id_number || orphan.id_number)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="إزالة"><UserMinus className="w-3.5 h-3.5" /></button>
                                              </div>
                                            </td>
                                          </tr>
                                        ))}
