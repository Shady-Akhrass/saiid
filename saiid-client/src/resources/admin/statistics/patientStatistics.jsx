...
const PatientsDashboard = () => {
    ...
    // Color schemas - Same as AidsDashboard
    const primaryColors = {
    ...
    useEffect(() => {
        ...
        animateValue(0, data.totalPatients, 2000, 'totalPatients');
        animateValue(0, data.recentPatientsCount, 2000, 'recentPatientsCount');
        animateValue(0, data.sickPatientsCount, 2000, 'sickPatientsCount');
        animateValue(0, data.healthyPatientsCount, 2000, 'healthyPatientsCount');
    }, [data.totalPatients, data.recentPatientsCount, data.sickPatientsCount, data.healthyPatientsCount]);
    ...
    const fetchDashboardData = async (silent = false) => {
        ...
        try {
            ...
            setData({
                ...
                healthStatusData: Object.entries(healthStatusCounts || {}),
                ...
                genderHealthBreakdownData: Object.entries(genderHealthBreakdown || {}),
                totalPatients: totalPatients || 0,
                averageBrothers: averageBrothers || 0,
                averageSisters: averageSisters || 0,
                averageSiblings: averageSiblings || 0,
                recentPatientsCount: recentPatientsCount || 0,
                sickPatientsCount: sickPatientsCount || 0,
                healthyPatientsCount: healthyPatientsCount || 0,
            });
        } catch (error) {
    ...
    const genderColors = (i, label) => {
        if (label === 'ذكر') return primaryColors.lightBlue;
        if (label === 'أنثى') return primaryColors.orange;
        return primaryColors.neutral;
    };

    const healthStatusColors = (i, label) => {
        if (label === 'مريض') return primaryColors.darkOrange;
        if (label === 'جيدة') return primaryColors.success;
        return primaryColors.neutral;
    };
    ...
    // Generate chart data
    const genderChartData = generateChartData('عدد المرضى حسب الجنس', data.genderData, genderColors, 'pie');
    const healthStatusChartData = generateChartData('عدد المرضى حسب الحالة الصحية', data.healthStatusData, healthStatusColors, chartType['health'] || 'bar');
    const maritalStatusChartData = generateChartData('عدد المرضى حسب الحالة الاجتماعية', data.maritalStatusData, maritalStatusColors, 'doughnut');
    const currentAddressChartData = generateChartData('عدد المرضى حسب العنوان الحالي', data.currentAddressData, addressColors, chartType['address'] || 'bar');
    const ageGroupsChartData = generateChartData('عدد المرضى حسب الفئة العمرية', data.ageGroupsData, ageGroupColors, 'bar');
    const genderHealthBreakdownChartData = generateChartData('توزيع الجنس والحالة الصحية', data.genderHealthBreakdownData, healthStatusColors, chartType['breakdown'] || 'bar');
        ...
        <div className={`min-h-screen bg-gradient-to-br from-blue-50 via-white to-orange-50 ${isDarkMode ? 'dark' : ''}`} dir="rtl">
            ...
            <div className="container mx-auto px-4 py-8 max-w-7xl">
                ...
                <div className="text-center mb-12 animate-fadeIn">
                    ...
                    <p className="text-gray-600 text-lg">نظرة شاملة على إحصائيات وبيانات المرضى</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    <StatCard
                        title="إجمالي المرضى"
                        value={animatedValues.totalPatients}
                        icon="🏥"
                        gradient="from-blue-400 to-blue-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`إجمالي المرضى: ${data.totalPatients}`, 'info')}
                    />
                    <StatCard
                        title="المرضى"
                        value={animatedValues.sickPatientsCount}
                        icon="🤒"
                        gradient="from-orange-400 to-orange-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`عدد المرضى: ${data.sickPatientsCount}`, 'info')}
                    />
                    <StatCard
                        title="الأصحاء"
                        value={animatedValues.healthyPatientsCount}
                        icon="💚"
                        gradient="from-green-400 to-green-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`عدد الأصحاء: ${data.healthyPatientsCount}`, 'info')}
                    />
                    <StatCard
                        title="سجلات حديثة (30 يوم)"
                        value={animatedValues.recentPatientsCount}
                        icon="📅"
                        gradient="from-purple-400 to-purple-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`السجلات الحديثة: ${data.recentPatientsCount}`, 'info')}
                    />
                </div>

                {/* Additional Stats Cards - Siblings Info */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    <StatCard
                        title="متوسط عدد الإخوة"
                        value={data.averageBrothers.toFixed(2)}
                        icon="👦"
                        gradient="from-blue-400 to-blue-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`متوسط عدد الإخوة: ${data.averageBrothers.toFixed(2)}`, 'info')}
                    />
                    <StatCard
                        title="متوسط عدد الأخوات"
                        value={data.averageSisters.toFixed(2)}
                        icon="👧"
                        gradient="from-orange-400 to-orange-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`متوسط عدد الأخوات: ${data.averageSisters.toFixed(2)}`, 'info')}
                    />
                    <StatCard
                        title="متوسط إجمالي الأشقاء"
                        value={data.averageSiblings.toFixed(2)}
                        icon="👨‍👩‍👧‍👦"
                        gradient="from-purple-400 to-purple-600"
                        isLoading={isLoading}
                        onClick={() => showNotification(`متوسط إجمالي الأشقاء: ${data.averageSiblings.toFixed(2)}`, 'info')}
                    />
                </div>
                ...
                <div className={`grid ${viewMode === 'grid' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'} gap-6`}>
                    ...
                    >
                        <Doughnut data={maritalStatusChartData} options={pieChartOptions} />
                    </ChartCard>
                    ...
                    <ChartCard 
                        title="توزيع الجنس والحالة الصحية" 
                        isLoading={isLoading}
                        chartId="breakdown"
                        chartData={genderHealthBreakdownChartData}
                        canToggleType={true}
                    >
                        {chartType['breakdown'] === 'line' ? (
                            <Line data={genderHealthBreakdownChartData} options={barChartOptions} />
                        ) : (
                            <Bar data={genderHealthBreakdownChartData} options={barChartOptions} />
                        )}
                    </ChartCard>
                </div>
