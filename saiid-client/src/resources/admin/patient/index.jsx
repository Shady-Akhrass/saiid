...
const Patients = () => {
  const { getData, setCachedData, isCacheValid, initializeCache, clearCache } = useCache('patients', 300000); // 5 دقائق
  const { invalidatePatientsCache } = useCacheInvalidation();
  ...
  useEffect(() => {
    const handleCacheInvalidation = (event) => {
      ...
      if (cacheKey === 'all' || cacheKey === 'patients') {
        clearCache();
        setRefreshTrigger(prev => prev + 1);

        if (import.meta.env.DEV) {
          console.log('✅ Patients cache invalidated, fetching fresh data');
        }
      }
    };
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      if (isCacheValid(filtersKey)) {
        ...
        if (cachedData) {
          setPatients(cachedData.patients || cachedData);
          setTotalPatients(cachedData.totalPatients || cachedData.length || 0);
          setTotalPages(cachedData.totalPages || Math.ceil((cachedData.totalPatients || cachedData.length) / perPage) || 0);
          setIsLoading(false);
          if (import.meta.env.DEV) {
      ...
      if (cachedData) {
        ...
        setIsLoading(false);
        if (import.meta.env.DEV) {
          console.log('⚠️ Error fetching patients, using cached data');
        }
        return;
      }
      ...
      if (!error.isConnectionError) {
        setToast({ message: "خطأ في جلب البيانات، يرجى المحاولة مرة أخرى.", type: "error", isVisible: true });
      }
    } finally {
      if (loadingTimeout) clearTimeout(loadingTimeout);
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    if (isCacheValid(filtersKey)) {
      ...
      if (cachedData) {
        ...
        setIsLoading(false);
        if (import.meta.env.DEV) {
          console.log('✅ Using cached patients data (from useEffect)');
        }
        return;
      }
    }
  ...
  const handleDownloadExcel = async () => {
    ...
    setToast({ message: "جاري تحضير ملف Excel للتحميل...", type: "info", isVisible: true });
    ...
    try {
      ...
      setToast({ message: "سيبدأ التحميل خلال لحظات...", type: "success", isVisible: true });
    } catch (error) {
      console.error("Error downloading Excel file:", error);
      setToast({ message: "يرجى المحاولة مرة أخرى بعد قليل", type: "info", isVisible: true });
    } finally {
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">إجمالي السجلات: { totalPatients }</p>
              </div>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-sky-50 to-orange-50 border-b border-sky-100">
                  ...
                  <th
                    onClick={ () => requestSort("current_address") }
                    className="p-4 text-center font-semibold text-gray-700 cursor-pointer hover:bg-sky-100/50 transition-colors duration-300 group"
                  >
                    <div className="flex items-center justify-center gap-2">
                  ...
                  <th
                    onClick={ () => requestSort("alternative_phone_number") }
                    className="p-4 text-center font-semibold text-gray-700 cursor-pointer hover:bg-sky-100/50 transition-colors duration-300 group"
                  >
                    <div className="flex items-center justify-center gap-2">
                      <Phone className="w-4 h-4 text-orange-500" />
                      <span>الهاتف البديل</span>
                    </div>
                  </th>
                </tr>
              </thead>
              <tbody>
                { isLoading ? (
                  ...
                  sortedPatients.map((patient, index) => (
                    <tr
                      key={ patient.id_number || patient._id || `${patient.name}-${index}` }
                    ...
                    >
                      <td className="p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            { patient.id_number && (
                              <p className="text-sm text-gray-500">الهوية: { patient.id_number }</p>
                            ) }
                          </div>
                        </div>
                      </td>
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ formatDate(patient.birth_date) }</span>
                      </td>
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ patient.current_address || "-" }</span>
                      </td>
                      <td className="p-4 text-center">
                        <span
                          className={ `px-3 py-1 rounded-full text-sm font-medium inline-flex items-center gap-1
                            ${patient.health_status === "جيدة"
                              ? "bg-gradient-to-r from-emerald-100 to-emerald-200 text-emerald-700"
                              : patient.health_status === "مريض"
                                ? "bg-gradient-to-r from-amber-100 to-amber-200 text-amber-700"
                                : "bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700"
                            }` }
                        >
                          <div
                            className={ `w-2 h-2 rounded-full ${patient.health_status === "جيدة"
                              ? "bg-emerald-500"
                              : patient.health_status === "مريض"
                                ? "bg-amber-500"
                                : "bg-gray-500"
                      ...
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ patient.guardian_phone_number || "-" }</span>
                      </td>
                      <td className="p-4 text-center">
                        <span className="text-gray-700">{ patient.alternative_phone_number || "-" }</span>
                      </td>
                    </tr>
                  ))
                ) }
